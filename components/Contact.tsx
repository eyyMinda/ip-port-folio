...
export default function Contact({ dark, pageInfo }: Props) {
  ...
  return (
    <div className="section">
      ...
      <div className="flex flex-col items-center mx-auto mt-20 space-y-4 text-center sm:mt-0">
        <h4
          className={`${!dark && "light"} h4
      max-w-[340px] sm:max-w-[100%]`}>
          Feel free to <span>Contact</span> me!
        </h4>

        <a
          className="text-lg contactIconWrap sm:text-2xl"
        ...
        <form
         ...
         flex flex-col space-y-2 w-[85%] sm:w-fit mx-auto`}>
          <div className="flex flex-wrap space-y-2 md:space-y-0 md:space-x-2 md:flex-nowrap">
            ...
            <input placeholder="Email" className="m-0 contactInput" type="email" name="email" required />
          </div>
          ...
          <button
            type="submit"
            className={`py-5 px-10 rounded-md font-bold ${dark ? "bg-primary-500" : "bg-secondary-400"}`}>
            Submit
          </button>
        </form>
      </div>
